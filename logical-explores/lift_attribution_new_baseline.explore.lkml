...
include: "/_standard-views/lift_attribution_new_baseline.layer.lkml"
...
include: "/_standard-views/ndt_orig_event_aggregates_new_baseline.view.lkml"
...
explore: lift_attribution_new_baseline {
  ...
  label: "Lift Attribution Data (new baseline)"

  access_filter: {
    field: lift_attribution_new_baseline.brand_id
    user_attribute: brand_id
  }

  always_filter: {
    filters: [
      lift_attribution_new_baseline.local_start_date_filter: "last 30 days",
      lift_attribution_new_baseline.airing_type: "1st Party",
      brands.brand_filter: ""
    ]
  }
  ...
  join: brand_central_products {
    type: left_outer
    sql_on: ${lift_attribution_new_baseline.sub_trigger_id} = CAST(${brand_central_products.orig_product_id} AS STRING);;
    relationship: many_to_one
  }

  join: brands {
    type: inner
    sql_on: ${lift_attribution_new_baseline.brand_id} = ${brands.brand_id};;
    relationship: many_to_one
  }

  join: dmas {
    type: left_outer
    sql_on: ${lift_attribution_new_baseline.dma_id} = ${dmas.dma_id};;
    relationship: many_to_one
  }

  join: spots {
    type: left_outer
    sql_on: ${lift_attribution_new_baseline.sub_trigger_id}=${spots.aeis_id} ;;
    relationship: many_to_one
    sql_where: ${spots.is_removed} = FALSE ;;
  }

  join: ndt_orig_event_aggregates_new_baseline {
    type: left_outer
    sql_on: ${lift_attribution_new_baseline.orig_event_id} = ${ndt_orig_event_aggregates_new_baseline.orig_event_id} and
  ...
  join: pdt_brand_startend_dates {
    type: left_outer
    sql_on: ${lift_attribution_new_baseline.brand_id} = ${pdt_brand_startend_dates.brand_id} ;;
    relationship: many_to_one
  }

  join: pdt_kinetiq_creatives {
    type: left_outer
    sql_on: ${lift_attribution_new_baseline.sub_trigger_id}=${pdt_kinetiq_creatives.creative_resource_id} ;;
    relationship: many_to_one
  }

  join: market_grouping {
    type: left_outer
    sql_on: ${lift_attribution_new_baseline.brand_id} = ${market_grouping.brand_id} AND
      ${lift_attribution_new_baseline.dma_id} = ${market_grouping.dma_id};;
    relationship: many_to_one
  }

  join: wwtv_programs {
    type: left_outer
    sql_on: ${lift_attribution_new_baseline.program_uuid} = ${wwtv_programs.program_uuid} and
            ${lift_attribution_new_baseline.year} = ${wwtv_programs.year} and
            ${lift_attribution_new_baseline.month} = ${wwtv_programs.month} and
            ${lift_attribution_new_baseline.day} = ${wwtv_programs.day};;
    relationship: many_to_one
  }
}
